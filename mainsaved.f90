program COSMO
   ...
   Call compute_solvent(solv_pot,param,solvent_sv,solvent_svt,solvent_area,T,200,0.001,solvent_sigma)
   !write(*,*) solv_pot
 ...
 !     end do
   ...
   contains
      ...
      subroutine calcgas(id_scr,gas_chem,area,sv,su,pot,ident,disp_con,param, T,r_cav)
         ...
         gas_chem=-id_scr-vdW_gain-thermo!-ring_corr
         write(*,*) gas_chem
      ...
      !  E_misfit=(1.88E11/2)*((sv1+sv2)**2.0_8)
         E_dd=E_hb+E_misfit
        ! write(*,*) E_dd
      ...
      subroutine iterate_solvent(pot_di,param,sv,svt,area,T,sigma)
         ...
         do j=1,size(sv)
            do i=-1,size(sv)
               temppot=temppot+(area(i)*dexp((-beta**(-1.0_8))*&
                      &E_dd(param(5),param(3),param(4),param(6),sv(j),svt(j),sv(i),svt(i))&
                      &+pot_di(i)))
              ! W_v(j)=W_v(j)+area(i)
              !  write(*,*) sigma(i),temppot, &
               !    &E_dd(param(5),param(3),param(4),param(6),dble(j),0.0_8,dble(i),0.0_8)
            end do
            !exit
            pot_di(j)=-log((sum(area)**(-1.0_8))*temppot)
            temppot=0.0_8
         end do
         deallocate(test_di)
         
         !write(*,*) beta*sum(pot_di)!-(param(8)*R*Jtokcal*T*log(sum(area)))
      ...
      !   write(*,*) temppot
         ...
         temp2=0
         do i=1,size(sol_pot)
            !do j=1,size(solv_pot)
            temppot=temppot+(area_sol(j)*solv_pot(j))
            !end do
         end do
         !do i=1,size(sol_pot)
       ...
       !  do i=1,size(sol_pot)
       !     do j=1,size(solv_pot)
       !        if (abs(sv_solv(i)-sv_sol(j)) .LE. 0.0005) then
       !           if (abs(svt_solv(i)-svt_solv(j)) .LE. 0.0005) then
       !               temp2=solv_pot(j)
       !               exit
       !           else
       !              temp2=0
       ...
       !     end do
         !   if (temp2 .EQ. 0) then
          !     write(*,*) "y"
          !  end if
          !  temppot=temppot+(area_sol(i)*temp2)
          !  temp2=0
        ! end do
         !write(*,*) temppot
